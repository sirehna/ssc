...
//! [LegTest example]
    ...
    // Construct the leg
    const ShortestPathLeg leg(boston, houston);
...
//! [LegTest length_example]
    ...
    const ShortestPathLeg boston_houston(boston, houston);
    const ShortestPathLeg houston_chicago(houston, chicago);
    const ShortestPathLeg houston_los_angeles(chicago, los_angeles);
    const ShortestPathLeg boston_los_angeles(boston, los_angeles);
...
TEST_F(ShortestPathLegTest, should_throw_if_attempting_to_find_a_point_outside_the_leg)
{
    for (size_t i = 0 ; i < 200 ; ++i)
    {
        ...
        // Construct the leg
        const ShortestPathLeg leg(point1, point2);
...
TEST_F(ShortestPathLegTest, distance_from_start_to_intermediate_waypoint_should_always_be_less_than_leg_length)
{
    for (size_t i = 0 ; i < 1000 ; ++i)
    {
        ...
        const LatitudeLongitude intermediate = leg.find_waypoint_at(d);
        ASSERT_LE(ShortestPathLeg(point1, intermediate).length(), leg_length);
    }
}
...
TEST_F(ShortestPathLegTest, distance_from_start_to_intermediate_waypoint_should_correspond_to_the_input)
{
    ...
    for (size_t i = 0 ; i < 1000 ; ++i)
    {
        ...
        const double d = a.random<double>().between(0, leg_length);
        const LatitudeLongitude intermediate = leg.find_waypoint_at(d);
        ASSERT_SMALL_RELATIVE_ERROR(ShortestPathLeg(point1, intermediate).length(), d, eps);
    }
}
...
TEST_F(ShortestPathLegTest, distance_between_miami_and_WP1_on_Norwegian_Epic_cruise_should_be_calculated_properly)
{
    ...
    ASSERT_NEAR(115.706, ShortestPathLeg(miami,wpA).length()/1e3, 1e-3);
}
