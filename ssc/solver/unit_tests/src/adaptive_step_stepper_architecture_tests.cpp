...
TEST_F(adaptive_step_stepper_architecture_tests, collaborators_should_be_called_in_the_right_sequence_for_adaptive_step_steppers_step_refused_and_no_event_detected)
{
    ...
    EXPECT_CALL(mock, has_more_time_events()).RetiresOnSaturation();
    solve_for_adaptive_step<ControlledStepperWithMock,SystemWithMock,ObserverWithMock<SystemWithMock>,EventHandlerWithMock>(sys,observer,stepper,scheduler,event_handler);
}
